...
type AuthState = AuthenticatedState | UnAuthenticatedState | LoadingState | GuestState
...
function createAuthState(setState: (state: AuthState) => void,discovery: DiscoveryDocument, token: string, refresh: string, expiry: number): AuthState {
...
function AuthScreen({ discovery, setAuthenticated }: { setAuthenticated: (state: AuthState) => void, discovery: DiscoveryDocument }) {
    ...
    useEffect(() => {
        async function authenticateUser() {
            ...
            setAuthenticated(createAuthState(setAuthenticated, discovery, token, refresh!, expiry))
        }
    ...
    return (
        <SafeAreaView style={{
            ...
            <Text onPress={guest} style={styles.guest}>Of ga door als gast</Text>
        </SafeAreaView>
    )
}
...
export function AuthProvider({ children }: { children: JSX.Element }) {
    const theme = useTheme()
    const discovery = AuthSession.useAutoDiscovery('https://leden.djoamersfoort.nl/o')
    ...
    useEffect(() => {
        ...
        async function getAuthenticated() {
            ...
            setAuthenticated(createAuthState(setAuthenticated, discovery!, token, refresh, parseInt(expiry)))
        }
    ...
    return (
        <AuthContext.Provider value={authenticated}>
            {discovery && authenticated.authenticated === Authed.UNAUTHENTICATED && <AuthScreen discovery={discovery} setAuthenticated={setAuthenticated} />}
            {discovery && authenticated.authenticated > Authed.UNAUTHENTICATED && children}
            {!discovery || authenticated.authenticated === Authed.LOADING && <View style={{
                ...styles.center,
                backgroundColor: theme.colors.primaryContainer
