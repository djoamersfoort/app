import {FlatList, StyleSheet, TouchableOpacity, Image, Alert} from "react-native";
...
export default function AlbumScreen({ route }: Props) {
    ...
    const [_cameraStatus, requestPermissions, getPermissions] = ImagePicker.useCameraPermissions();
    ...
    async function upload(images: ImagePicker.ImagePickerAsset[]) {
        ...
        images.forEach(asset => {
            // @ts-ignore
            formData.append('items', { uri: asset.uri, name: 'file', type: asset.mimeType })
        })

        await fetch(`https://media.djoamersfoort.nl/api/items/${route.params.album}`, {
            method: 'POST',
            headers: {
    ...
    async function captureImages() {
        ...
        if (!permissions.granted) {
            if (!permissions.canAskAgain) return Alert.alert('Camera permissions denied!', 'Change permissions in your device\'s settings')

            const result = await requestPermissions()
            if (!result.granted) return Alert.alert('Camera permissions denied!', 'Change permissions in your device\'s settings')
        }
